                    ...
                    <MathJax.Provider>
                        <div>
                            ...
                            <p style={{textAlign:"left"}}>
                                ...
                                since <MathJax.Node inline formula={`\\delta^{\\beta - \\alpha} \\to 0 \\text{ as } \\delta \\to 0`} /> and the other part of the inf is finite. For <MathJax.Node inline formula={`\\beta < \\alpha`} />, we get the same thing but a factor of <MathJax.Node inline formula={`\\delta^{\\beta - \\alpha}\\text{ }`} /> in the other direction, which goes to infinity as <MathJax.Node inline formula={`\\delta \\to 0`} />. 
                            ...
                            <br/><br/>
                            <p style={{textAlign:"left"}}>
                                The Cantor set is defined as <MathJax.Node inline formula={`C = \\cap_{i=0}^{\\infty} \\cup_{j=0, j \\neq 1 \\mod{3}}^{3^{i}} [\\frac{j}{3^{i}}, \\frac{j+1}{3^{i}}]` } />. This is a nowhere dense set with measure zero. An example of a fractal with integer Hasudorff dimension is the boundary of the Mendelbrot set, which has Hausdorff dimension 2:
                            </p>
                            ...
                            <p style={{textAlign:"left"}}>
                                ...
                                interesting questions one can ask if we restrict ourselves a bit: Define the well-behaved fundamental group <MathJax.Node inline formula={`\\pi_{U}^{wb}(x_{0}) = \\{[f]: I \\to U | f(0) = x_{0},\\text{ }f\\text{ continuous, and } \\dim_{H}(K) = 1\\text{ } \\forall{K \\subset f(I)}\\text{ open} \\}`} />. This well-behaved fundamental group is fundamentally different from the usual fundamental group (pun intended) since being path-connected 
                            ...
                            <p style={{textAlign:"left"}}>
                                We now turn to the whole point of us understanding what the Hausorff dimension of a suitable subspace of <MathJax.Node inline formula={`R^{n}`} />; we say an attractor <MathJax.Node inline formula={`\\Lambda`} /> is strange if the dynamical system <MathJax.Node inline formula={`(\\Lambda, f)`} /> is chaotic and <MathJax.Node inline formula={`\\Lambda`} /> is a fractal. 
